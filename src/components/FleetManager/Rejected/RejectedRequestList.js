...
// create a component
class FleetManagerRejectedRequestView extends React.Component {
    ...
    refreshFetchList() {
        ...
        fetch(this.state.baseUrl + this.state.apiRoute + 'fleetmanager/fetch/assigned/' + this.state.userId + "/requests/rejected", {
            method: 'GET',
            headers: {
            ...
            .then((responseJson) => {
                ...
                //console.log("my response for refresh fetch fleet manager rejected requests by ID api is--->>>")
    ...
    fetchList() {
        ...
        fetch(this.state.baseUrl + this.state.apiRoute + 'fleetmanager/fetch/assigned/' + this.state.userId + "/requests/rejected", {
            ...
            .then((responseJson) => {
                ...
                //console.log("my response for fetch fleet manager rejected requests by ID api is--->>>")
                //console.log(responseJson)
    ...
    renderItem = ({ item }) => (
        ...
        <ListItem titleNumberOfLines={1} onPress={this.viewDetails.bind(this, item)}>
            ...
            <Badge
                value={"Rejected"}
                badgeStyle={{ backgroundColor: '#9a999b' }}
                textStyle={{ color: '#FFF' }}
    ...
    viewDetails(rowData) {
        this.props.navigation.navigate('FleetManagerDetailsPage', { apiListItemData: rowData, headerBarColor: "#4c4c4c", refreshFetchList: () => this.refreshFetchList() });
    }
...
export default FleetManagerRejectedRequestView;